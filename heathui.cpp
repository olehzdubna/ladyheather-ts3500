...
//    if(user_set_osc_float == 0) plot[OSC].float_center = 1;
      ...
      plot[OSC].float_center = 0;

      plot[DAC].show_plot = 0;
      plot[DAC].float_center = 0;
...
//          if(res_t == RES_T_SMT) com[RCVR_PORT].parity = NO_PAR;
         ...
         else if((e == 's') && (f == 't')) { // /rxst - Symmetricom TimeSource
            rcvr_type = TS3500_RCVR;

            printf("+++ RECEIVER TYPE: %lx\n", rcvr_type);
         }
